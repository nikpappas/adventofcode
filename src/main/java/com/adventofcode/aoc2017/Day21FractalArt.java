...
//        trans.put(".#./..#/###","#..#/..../..../#..#");
    ...
    public static void main(String[] args) {
        String input = INPUTS + "/day21.txt";
        Day21FractalArt app = new Day21FractalArt(input);

        app.solve(5);
//        Part 2
        app = new Day21FractalArt(input);
        app.solve(18);
    }
    ...
    private void solve(int i) {
        ...
        int on = map.stream().map(this::countHash).reduce((x, y) -> x + y).orElseThrow(RuntimeException::new);
...
//                System.out.println(medium);
    ...
    String flipRot(String code, int i) {
        if (code.length() == 5) {
...
//                System.out.println(medioum);
    ...
    String flipInvRot(String code, int i) {
        if (code.length() == 5) {
...
//                System.out.println(medioum);
    ...
    private static Map<String, String> parse(String path) {
        return Utils.readFile(path)
                .map(line -> line.split(" => "))
                .collect(Collectors.toMap(toks -> toks[0], toks -> toks[1]));
    }
