...
//        System.out.println(app.regs);
        String input = INPUTS + "/day18.txt";
        Day18Duet p1 = new Day18Duet(input, 1);
        Day18Duet p2 = new Day18Duet(input, 2);
        p1.setOther(p2);
        p1.regs.put("p", 0L);
        p2.setOther(p1);
        p2.regs.put("p", 1L);
...
//        System.out.println(threadId+"paused having sent: " + sent);
    ...
    static List<String> parseInstructions(String path) {
        return Utils.readFile(path).flatMap(l -> Arrays.stream(l.split(","))).collect(Collectors.toList());
    }
